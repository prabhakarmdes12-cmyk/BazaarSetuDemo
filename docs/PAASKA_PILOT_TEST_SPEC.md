...
**Operating Architecture:** Paaska Consumer Operating System (Powered by Chiti Technologies)  
...
```text
INTENT → BASKET → QUOTE → ORDER → PAYMENT → FULFILMENT → HANDOFF → COMPLETION
```
...
| **SLA Thresholds** | 2 min: Merchant reminder · 3 min: Chiti Console operator escalation |

---

## 3. The 50 Hostile Transactions Test Matrix

### Phase A: 10 Canonical Happy Path Orders
...
| **HP-05** | Regular customer credit order | 10-Min Delivery | UDHAAR | Credit balance checked → Ledger debit added → Delivered | Khata balance updated, DSO tracked |
...
| **HP-08** | 1-Tap "Buy Again" repeat | 10-Min Delivery | DIRECT_UPI | Customer taps "Repeat" on HP-01 → Cart repopulated → Order placed | Sub-30s re-order time |
| **HP-09** | Large festive pooja basket | 10-Min Delivery | DIRECT_UPI | 8 pooja items + ghee + makhana → Packed in one carton → Delivered | All 8 line items verified |
| **HP-10** | Late-night self-pickup hold | Self Pickup | COD | Placed at 10 AM → Customer picks up at 6 PM (within 12h window) | Order remains valid and retrievable |

---

### Phase B: 40 Edge-Case & Failure Scenarios (The Hostile Tests)

#### Category 1: Merchant Availability & SLA Breaches (HT-01 to HT-08)
- **HT-01: Merchant No-Response (SLA Breach)** — Order placed; merchant phone unattended. At 2m, audio alert sounds. At 3m, draft flags `NEEDS_OPERATOR_ASSIST` in Chiti Console Radar. Operator triggers WhatsApp/call outreach.
...
- **HT-04: Merchant Slow Packing (>15 mins)** — Status remains in `preparing` for 15 mins. Customer tracking timeline shows reassurance: *"Aapka taaza samaan pack ho raha hai"*.
...
- **HT-06: Multiple Rapid Orders (Rush Hour)** — 5 orders placed within 30 seconds. Vendor Action Inbox renders all 5 sorted by urgency timer.
- **HT-07: Merchant Offline at Placement** — Merchant socket disconnected. Order queues; SMS push triggered to merchant phone.
- **HT-08: Merchant Toggles Shop Inactive** — Bighi Brothers sets `isActive = false`. Customer storefront renders "Currently Closed" banner; Add to Cart disabled.

#### Category 2: Inventory & Substitution Failures (HT-09 to HT-16)
- **HT-09: Single Item Out of Stock** — 3 items ordered; paneer missing. Merchant proposes Amul Cheese as substitute. Customer receives notification card.
- **HT-10: Customer Accepts Substitute** — Customer clicks "Accept" on substitute. Basket updates to new total; status returns to `preparing`.
...
- **HT-12: Unmatched Product Query** — Customer types "Special Dhanbad Gur Rewari". Matcher assigns `NEEDS_MERCHANT_CHECK`. No price is fabricated. Merchant quotes price manually.
- **HT-13: Price Discrepancy at Counter** — MRP increased from ₹44 to ₹46. Merchant adjusts price in DraftAdjustment. Customer prompted to approve ₹2 difference before dispatch.
...
- **HT-15: Ambiguous Unit Parsing** — Voice query: "Thoda sa zeera de do". System flags unit ambiguity; prompts customer: "50g ya 100g?".
...
#### Category 3: Customer Cancellation & Address Edge Cases (HT-17 to HT-24)
- **HT-17: Customer Cancels Within 60s** — Customer clicks "Cancel Order" while status is `pending`. Order transitions to `rejected` with reason `CANCELLED_BY_CUSTOMER`.
- **HT-18: Customer Tries to Cancel After Packing** — Order is in `ready` or `out_for_delivery`. Cancel button disabled; directs customer to call shopkeeper.
- **HT-19: Delivery Outside Serviceable Pincode** — Customer enters PIN 828101 (outside pilot radius). Pilot locality guard blocks checkout with clear message: *"Abhi kewal Bank More & Dhanbad (826001) mein uplabdh hai"*.
- **HT-20: Missing Street Address** — Customer selects 10-Min Delivery with blank line1. Address validation sheet blocks order placement until house/street is provided.
- **HT-21: Self-Pickup Selected Without Address** — Customer selects Self Pickup with zero saved addresses. System allows checkout; sets destination to Dukaan Counter; zero address error.
...
- **HT-23: Delivery Address Change Request** — Customer chats mid-transit: "Bhaiya Flat 301 nahi, Flat 402 mein aao". Chitigram chat delivers message directly to merchant view.
...
- **HT-27: Customer Forgets Phone at Pickup** — Customer has no phone. Merchant verifies customer name and order number against Action Inbox.
- **HT-28: Pickup Order Exceeds 12 Hours** — Order placed at 8 AM, unclaimed by 9 PM. Console alerts merchant to return perishables to shelf or call customer.
...
- **HT-32: Payment Discrepancy ("Paid" vs "Unpaid")** — Customer says paid; merchant bank statement shows no credit. System allows merchant to flag `PAYMENT_DISPUTE`; auto-logs in Chiti Console.
